...
def get_efficiency_meta(efficiency):
    if efficiency >= 80:
        return {
            "label": "Excellent",
            "status_ar": "ممتاز",
            "color": "#0f9d7a",
            "bg": "#e9fbf5",
            "message": "أحسنت! جلسة غسل اليدين أظهرت كفاءة عالية في استخدام المياه.",
            "meaning": "يشير الفيديو إلى تحكم جيد في تدفق المياه مع هدر محدود جدًا.",
            "impact": "هذا يعني أن نمط استخدامك أقرب إلى السلوك اليومي المستدام في استهلاك المياه.",
            "advice": "استمر على هذا السلوك الجيد، وحافظ على فتح الماء فقط عند الحاجة الفعلية."
        }
    elif efficiency >= 50:
        return {
            "label": "Moderate",
            ...
            "color": "#f39c12",
            "bg": "#fff6e7",
            "message": "استخدامك للمياه متوسط ويمكن تحسينه.",
            "meaning": "يبدو أن الماء بقي مفتوحًا خلال بعض اللحظات التي كان فيها النشاط منخفضًا.",
            "impact": "الهدر البسيط المتكرر في العادات اليومية قد يتحول إلى استهلاك كبير مع الوقت.",
            "advice": "حاول إغلاق الماء أثناء فرك اليدين لتقليل التدفق غير الضروري."
        }
    else:
        return {
            ...
            "color": "#e74c3c",
            "bg": "#fdeceb",
            "message": "مستوى هدر المياه مرتفع ويحتاج إلى تحسين.",
            "meaning": "تشير النتيجة إلى أن الماء بقي مفتوحًا لفترات أطول من اللازم أثناء لحظات غير نشطة.",
            "impact": "هذا النمط قد يؤدي إلى هدر يومي متكرر يمكن تجنبه بسهولة.",
            "advice": "أغلق الماء أثناء فرك اليدين، ثم أعد فتحه فقط عند الشطف."
        }
...
def analyze_video(path):
    ...
    if duration > max_duration:
        cap.release()
        return {"error": "الفيديو طويل جدًا، الحد الأقصى المسموح هو 15 ثانية."}
    ...
    try:
        ...
        return {"error": f"حدث خطأ أثناء تحليل الفيديو: {str(e)}"}
    finally:
        cap.release()
    ...
    efficiency = round(100 - waste_percentage, 1) if processed_frames >= 1 else 0
    ...
    score_meta = get_efficiency_meta(efficiency)
    estimated_saving_liters = round(wasted_water * 0.6, 2)
    ...
    return {
        ...
        "message": score_meta["message"],
        "meaning": score_meta["meaning"],
        "impact": score_meta["impact"],
        "advice": score_meta["advice"],
        "score_label": score_meta["label"],
        "status_ar": score_meta["status_ar"],
        "score_color": score_meta["color"],
        "score_bg": score_meta["bg"],
        "estimated_saving_liters": estimated_saving_liters
    }
